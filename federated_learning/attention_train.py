...
class Encoder(nn.Module):
    ...
    def forward(self, src):
        ...
        expand_temp = src.reshape(src.shape[0], src.shape[1], 1)
...
class Decoder(nn.Module):
    def __init__(self, output_dim, emb_dim, enc_hid_dim, dec_hid_dim, dropout, attention):
        ...
        self.rnn = nn.GRU(enc_hid_dim + 1, dec_hid_dim)
        self.fc_out = nn.Linear(enc_hid_dim + dec_hid_dim + 1, output_dim)
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        expand_output = input.reshape(1, 1, 1)
...
def main():
    with open("src/train_para.pl", "rb") as fp:   # Unpickling
        raw = pickle.load(fp)
    
    input_data = []
    for round in range(len(raw)):
            for series in raw[round]:
                input_data.append(series.cpu().detach().numpy())
    random.shuffle(input_data)
    train_data = np.array(input_data[25:])
    print('shape of train_data is')
    print(train_data.shape)

    val_data = np.array(input_data[0:25])
    print('shape of val_data is')
    print(val_data.shape)

    with open("src/corrupt_para.pl", "rb") as fp:   # Unpickling
        raw_corrupt = pickle.load(fp)
    
    corrupt_data = []
    for round in range(len(raw_corrupt)):
            for series in raw_corrupt[round]:
                corrupt_data.append(series.cpu().detach().numpy())

    corrupt_data = np.array(corrupt_data[0:25])
